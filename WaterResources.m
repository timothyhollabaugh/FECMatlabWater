...
if Latitude>=0
    ...
    acos(-tan((Latitude-Beta))*tan(delta))); % Sunset angle on panels in North hemisphere
else
    ws=min(acos(-tan(Latitude)*tan(delta)),...
    acos(-tan(Latitude+Beta)*tan(delta))); % Sunset angle on panels in South hemisphere
end
...
% Extra-terrestial insolation on a normal surface
Gon=Gsc*(1+0.033*cos(MeanDay*2*pi/365));

% Daily Extra-terrestial solar energy (kWh/m^2/day) on a horizontal surface
Ho=24*Gon/pi.*(cos(Latitude)*cos(delta).*sin(wsh)+wsh*sin(Latitude).*sin(delta));
...
alpha=-0.004; % Temperature coefficient (Change in power per cell temp. above 25 C)
...
while AccumulatedCost_Generator(end)<InitialCost_Solar(end)
    N=N+1; % When the loop end the value of N is the years until the generator NPV is equal or greater than the solar system NPV
    ...
    YearlyCost_Generator(N+1)=YearlyCost_Fuel^N; % Yearly % of fuel with discounted value of money
    AccumulatedCost_Generator(N+1)=AccumulatedCost_Generator(N)+YearlyCost_Generator(N+1); %accumulated generator cost
end
